...
def main():
    try:
        ...
        elif not sys.argv[1].lower().endswith(acceptable_suffixes) or not sys.argv[2].lower().endswith(acceptable_suffixes):
            sys.exit("Invalid input.")
        elif sys.argv[1].lower().split(".")[1] != sys.argv[2].lower().split(".")[1]:
            sys.exit("Input and output have different extensions.")
        else:
            file_handler(sys.argv[1], sys.argv[2])
    except Exception as e:
        sys.exit(f"Error: {e}")


def file_handler(file_input, save_file):
        try:
